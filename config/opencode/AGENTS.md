...
When running shell commands, prefer RTK wrappers for supported high-output commands when compact output is enough.
...
Use raw commands when exact full output is required, when RTK does not support the command, or when the wrapper would obscure needed details.
