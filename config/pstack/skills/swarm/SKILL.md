...
4. Pick the worker model from `swarm workers` in `~/.config/pstack/models/<runtime>.md` when present. Otherwise use `grok-4.6-fast-xhigh`. For a model race, name each arm's model up front.
