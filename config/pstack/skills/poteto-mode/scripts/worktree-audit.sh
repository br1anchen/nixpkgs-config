...
# Supply only a transcript directory already scoped to this repository.
...
git worktree list --porcelain | sed -n 's/^worktree //p' | while read -r wt; do
	...
	if [ -d "$transcripts" ]; then
        f=$(python3 - "$transcripts" "$wt" <<'SCAN'
import datetime, pathlib, sys
root, worktree = pathlib.Path(sys.argv[1]), sys.argv[2]
matches = []
for path in root.rglob('*.jsonl'):
    with path.open(errors='replace') as stream:
        if any(worktree + '/' in line or worktree + '"' in line for line in stream):
            matches.append(path.stat().st_mtime)
if matches:
    stamp = int(max(matches))
    print(stamp, datetime.datetime.fromtimestamp(stamp).strftime('%Y-%m-%d'))
SCAN
        )
	...
	case "$dirty" in wip:*) bucket=hold-wip ;; *)
		case "$pr" in *OPEN*) bucket=hold-open-pr ;; *)
			...
			elif [ ! -d "$transcripts" ]; then bucket=review-no-transcripts
