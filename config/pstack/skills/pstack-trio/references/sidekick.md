...
master owns scope, review, and the human. A consultant of another agent kind
advises the master in a third pane; it reads your reports and progress log
from the store, never writes the tree, and never messages you. Everything you
say that matters goes in a report file; the terminal is not the record.
...
cons. The consultant, in parallel, critiques the design itself. You
brainstorm against the alternatives and check whether the plan survives
contact with the code. Your answer is your real judgment, not agreement by
default, and the decision stays with the master.
...
4. `pair.sh notify <store> <report>`, then end the turn with the single line
   `pstack-trio REPORT <path>`. A new plan round arrives as a new PLAN message.

## On `pstack-trio BRIEF <path>`

1. Read the brief. A missing or unfillable field means a report with status
   `blocked` and the gap under Questions, before any work. Files under
   `<store>/scratch/` are the consultant's worktrees; leave them alone. A feature, bug-fix,
...
5. `pair.sh notify <store> <report>`, then write the single line
   `pstack-trio REPORT <path>`. The master's wait sees the report file land, and
   notify covers the case where it was not waiting.
6. When the report's status is `done`, run `pair.sh next <store>`. If it
   prints `pstack-trio BRIEF <path>`, the master queued that brief while you
...
- Object: leave the tree consistent, then write `reports/NNN-<slug>-s<k>.md`
  ...
  `pstack-trio REPORT <path>`. The master's answer arrives as the next STEER, a
...
- Questions for the human travel in the report under Questions. The master
  asks them. Questions for the consultant do not exist: the master consults.
