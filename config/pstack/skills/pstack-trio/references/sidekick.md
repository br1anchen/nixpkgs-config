...
1. Read the brief. A missing or unfillable field means a report with status
   ...
   `<store>/scratch/` are the consultant's worktrees; leave them alone. A
   feature, bug-fix, refactoring, perf-issue, or pstack-tdd brief whose `plan:`
   line does not point at a plan you agreed to is the same: report `blocked`
   and name the plan round you would need first.
