...
4. **Design and plan together.** You drive the implementation design, the
   ...
   design, the alternatives with their pros and cons, one block per future
   brief, each a slice that lands on its own and is split into commit-sized
   steps with their checks, verification, risks, open questions. Send it with
   `pair.sh discuss <store> <plan>`. It reaches the sidekick and the consultant
   at once and returns both responses: the sidekick's report on whether the
   plan survives the code, and the consultant's advice on whether the design is
...
5. **Brief.** `pair.sh new-brief <store> <unit-slug>` and fill every field. Set
   ...
   unit start from a commit, and put the landing gate's fast checks (format,
...
6. **Dispatch and check in.** `pair.sh dispatch <store> <brief> [--every MIN]`.
   ...
   waits in the queue. Any consult may run while the sidekick works.
7. **Review.** Read the report, then read the unit's commit yourself: `git show
   ...
   after the verdict. When the unit has steps, the report comes with
   `review-delta:`: read only that range, since the notes covered the rest and
   `finish` made sure their blocking items were resolved. Split what you find
   into Blocking and Follow-ups as the review template says; only a blocking
   finding makes a revise, and follow-ups go to `followups.md`. Route through
   the review skills the change warrants: `blast-radius` for a small diff you
   distrust, `no-comments` before accepting, `interrogate` for a contested
   design. When the verdict is contested in your own reading, or the unit is on
   its second revise, send a review consult and read the advice before you
   decide. Write `reviews/NNN-<slug>.md` from the review template with one
   verdict, the advice it drew on, and any overrule. Log one `pair.sh log` row
   per verdict.
8. **Loop.** `revise` becomes the next brief with the review path in Context;
   ...
   and steer the running unit only when the findings invalidate it. A unit's
   third revise is not sent: accept what is safe with the rest as follow-ups,
   or take the unit back to a plan round. `accept` moves to the next unit. A
   finding that changes the design, not just one unit, goes to a consult and
...
- A glance is optional and cheap: five minutes on the diff at a check-in,
  asking whether the review would say revise. Send one on the first step of a unit whose revise would
  cost the most, or at a check-in showing a sizeable diff on a unit without
  steps. The consultant's quota is the scarce one; skip it on small or
  mechanical units.
...
2. Progress lines naming an approach the agreed plan did not choose, or a step
   the plan skipped.
...
When a wait prints `paused:` or a command refuses because the store is
paused, finish the step you are in and start nothing new. Write where the
work stands and the exact next step in `gates.md`, log a row, and end your
turn. After `pair.sh resume`, re-read this skill if it changed, have the
sidekick re-read its role file, and re-dispatch the paused brief with its
partial report in Context.
