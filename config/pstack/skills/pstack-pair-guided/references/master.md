...
5. **Direction and approval.** Set the plan's `scale` honestly and take the
   ...
   proceed. Large: paste it into your reply, run
   `herdr notification show "pstack-pair-guided: direction needs approval" --body "<slug>" --sound request`,
   add a `gates.md` entry, end your turn, and set `approval: human` only after
   the human approves. If the human redirects, that is a new plan round.
   Dispatch checks approval against scale.
6. **Brief.** `pair.sh new-brief <store> <unit-slug>` and fill every field.
...
9. **Loop.** `revise` becomes the next brief with the review path in Context.
   ...
   orders.
10. **Close.** When the predicate holds on the real artifact, run
