#!/usr/bin/env bash
# Pair store and Herdr channel helper for the pstack-pair-guided skill.
...
cmd_spawn() {
	...
	local bootstrap="Load the pstack-pair-guided skill from ~/.agents/skills/pstack-pair-guided/SKILL.md and take the sidekick role. Pair store: $store. Follow its bootstrap steps, write reports/000-ready.md, and reply READY."
...
send_and_wait() {
	...
	if grep -q '{{' "$file" || grep -qE '^(decided by|status): .*\|' "$file"; then
...
cmd_notify() {
	...
	case "$status" in
	idle | done)
		herdr agent prompt "$master" "pstack-pair-guided REPORT $report" >/dev/null
...
cmd_stop() {
	...
	errfile="$(mktemp)"
	out="$(herdr agent prompt "$name" "pstack-pair-guided STOP $store" --wait --timeout "$timeout" 2>"$errfile")" || code=$?
	err="$(cat "$errfile")"
	rm -f "$errfile"
...
cmd_status() {
	...
	local store="$1" master sidekick f seq slug kind report review rstatus verdict units k
	master="$(field "$store" .master.name)"
